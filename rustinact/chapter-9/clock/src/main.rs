...
const NTP_TO_UNIX_SECONDS: i64 = 2208988800;
...
impl NTPMessage {
    ...
    fn client() -> Self {
        const VERSION: u8 = 0b00011000;
        const MODE: u8 = 0b00000011;
...
fn check_time() -> Result<f64, std::io::Error> {
    ...
    for time in &times {
        ...
        let delay = time.delaty() as f64;

        let weight = 1000000.0 / (delay * delay);
        if weight.is_finite() {
            offsets.push(offset);
...
impl Clock {
    ...
    fn set<Tz: TimeZone>(t: DateTime<Tz>) -> () {
        ...
        let is_leap_second = ns > 1000000000;
        if is_leap_second {
            ns -= 1000000000;
        }
        ...
        systime.wMilliseconds = (ns / 1000000) as WORD;
...
fn main() {
    let app = App::new("clock")
        ...
        .after_help("Note: UNIX timestamp are parsed as whole seconds since 1st january 1970 0:00:00 utc. for more accuracy, use another format")
        .arg(
