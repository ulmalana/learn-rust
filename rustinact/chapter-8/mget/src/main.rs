...
fn main() {
    let app = App::new("mget")
        ...
        .arg(Arg::with_name("url").require_equals(true))
    ...
    if url.scheme() != "http" {
        eprintln!("error: only http supported");
        return;
    }

    let tap = TapInterface::new(&tap_text)
        .expect("error: unable to use <tap-device> as network interface");

    let domain_name = url.host_str().expect("domain name required");

    let _dns_server: std::net::Ipv4Addr = dns_server_text
        .parse()
        .expect("error: unable to parse <dns-server> as IPv4 address");
